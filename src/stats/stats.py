...
def update_stats(individuals, end):
    ...
    # if stats['gen'] == 5:
    #     f = open("best_digits.txt",'a')
    #     f.writelines(str(trackers.best_ever.phenotype)+'\t')
    #     f.writelines(str(trackers.best_ever.fitness) + '\t')
    #     f.writelines(str(stats['total_time']) + '\t')
    #     f.writelines(str(stats['unique_inds']) + '\n')
    ...
    # for o, ff in \
    #     enumerate(params['FITNESS_FUNCTION'].fitness_functions):
    #     stats['ave_fitness' + str(o)] = trackers.best_fitness_list
    # fitnesses1 = []
    # fitnesses2 = []
    # fitnesses2 = []
    # for o, ff in \
    #                 enumerate(params['FITNESS_FUNCTION'].fitness_functions):
    #                 fitnesses1 = [i[0] for i in trackers.best_fitness_list]
    #                 fitnesses2 = [i[1] for i in trackers.best_fitness_list]
    #                 fitnesses2 = [i[2] for i in trackers.best_fitness_list]

    # stats['ave_fitness0'] = np.nanmean(fitnesses1, axis=0)
    # stats['ave_fitness1'] = np.nanmean(fitnesses2, axis=0)
    # stats['ave_fitness2'] = np.nanmean(fitnesses2, axis=0)
    # stats['ave_fitness'] = fitnesses
...
def print_first_front_stats():
    ...
    if stats['gen'] == 0:
            f = open("multi_encorr_pareto_0.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
    elif stats['gen'] == 1:
            f = open("multi_encorr_pareto_1.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
    elif stats['gen'] == 2:
            f = open("multi_encorr_pareto_2.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
    elif stats['gen'] == 3:
            f = open("multi_encorr_pareto_3.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
    elif stats['gen'] == 4:
            f = open("multi_encorr_pareto_4.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
    elif stats['gen'] == 5:
            f = open("multi_encorr_pareto_5.txt",'a')
            f.writelines(str(first_pareto_list)+'\n') 
            f.close()
...
def print_final_moo_stats():
    ...
    for ind in trackers.best_ever:
        ...
        if stats['gen'] == 5:
            f = open("best_multi_en_corr.txt",'a')
            f.writelines(str(ind)+'\n') 
            # f.writelines(str(trackers.best_ever.fitness) + '\t')
            # f.writelines(str(stats['total_time_many']) + '\t')
            # f.writelines(str(stats['unique_inds_many']) + '\n')
            f.close()
